...
const {UnprocessableEntity} = require('throw.js');

module.exports = function (app) {
  let router = express.Router();
  router.post('/', function (req, res, next) {
    if (!req.body.type) {
      return next(new UnprocessableEntity('Type is required'));
    }
    findConsumer(req.body.consumerId)
      .then(consumer => {
        next(new Error('haha'));
        if (!consumer) {
          next(new UnprocessableEntity('Consumer Not Found: id:' + req.body.consumerId));
        }
