...
module.exports.bootstrap = function (app) {
  ...
  if (httpServer && config.gatewayConfig.http.enableWebSockets) {
    httpServer = interceptUpgrade(httpServer, app);
  }

  if (httpsServer && config.gatewayConfig.https.enableWebSockets) {
    httpsServer = interceptUpgrade(httpsServer, app);
  }
...
function interceptUpgrade(server, app) {
